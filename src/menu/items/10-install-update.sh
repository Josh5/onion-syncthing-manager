...
registry_package_json=$(wget --header="Accept: application/vnd.github+json" -O - -o /dev/null https://api.github.com/repos/syncthing/syncthing/releases/latest)
...
if [ -f "${appdir:?}/share/syncthing/.current_version" ]; then
    ...
    if [ "${current_version:-}" = "${latest_package_version:?}" ]; then
        ...
        echo "    If you wish to re-install syncthing, first run"
        echo "    the current binary in the ./bin directory."
...
if [ -f "$versioned_tar" ]; then
    ...
    curl -L -o "$tmp_tar" "${latest_package_url:?}" || {
